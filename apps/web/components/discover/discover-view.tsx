...
import { useDiscoverPeople, useDiscoverGroups, useTrendingPosts } from '@/hooks/use-social';
      ...
      {people.map((p: any) => (
        <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-4 hover:shadow-md transition-shadow">
          ...
          <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-50 text-[11px]">
            <span className="text-gray-400">{p.followerCount ?? 0} followers</span>
            <button
              onClick={async () => {
                try {
    ...
    <ul className="space-y-3">
      {posts.map((p: any) => (
        <li key={p.id} className="bg-white rounded-2xl border border-gray-100 p-4">
          ...
          <p className="text-[11px] text-gray-400 mt-2">{p.likeCount} likes • {p.commentCount} comments • {p.saveCount} saves</p>
        </li>
      ))}
    </ul>
  );
